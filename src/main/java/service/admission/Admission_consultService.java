...
import dao.admission.Admission_consultDAO;
import dto.admission.Admission_consultDTO;
	 ...
	 * 내용 : 입학안내 - 입학상담 서비스
	 */
	public enum Admission_consultService {
	
		INSTANCE;
	
		private Admission_consultDAO dao = Admission_consultDAO.getInstance();
	
		public void register(Admission_consultDTO dto) {
			dao.insert(dto);
		}
	
		public Admission_consultDTO findByPass(int id) {
			return dao.select(id);
		}
	
		public List<Admission_consultDTO> findAll(int start) {
			return dao.selectAll(start);
		}
		
		public List<Admission_consultDTO> findAllSearch(int start, String searchType, String keyword) {
			return dao.selectSearch(start, searchType, keyword);
		}
	
		public void modify(Admission_consultDTO dto) {
			dao.update(dto);
		}
